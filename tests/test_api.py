...
def test_item_add_unsafe_url(client, auth_headers):
    with patch("app.api.endpoints.item.check_url_safety", return_value=(False, "Phishing site detected")):
        payload = {
...
def test_settings_toggle_logging(client, auth_headers):
    ...
    response = client.post("/settings/logging", json={"enabled": True}, headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["success"] is True
    assert "enabled" in response.json()["message"]
    
    # Test disabling logging
    response = client.post("/settings/logging", json={"enabled": False}, headers=auth_headers)
    assert response.status_code == 200
    assert response.json()["success"] is True
