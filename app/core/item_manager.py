...
class ItemManager:
    ...
    def add_item(self, url: str, size: str, interval_seconds: int) -> tuple[bool, str]:
        with self.lock:
            ...
            self._write_data(data)
            return True, f"New product added with ID {new_id}, size {size} and interval {interval_seconds}s."

    def update_item(self, item_id: str, new_url: str, new_interval_seconds: int) -> tuple[bool, str]:
        with self.lock:
            data = self._read_data()
    ...
    def delete_item(self, url: str, size: str) -> tuple[bool, str]:
        with self.lock:
            data = self._read_data()
            ...
            if not sizes:
                del data[item_id]
                self._write_data(data)
                return True, f"Last size removed. Product with ID {item_id} has been completely deleted."
            else:
