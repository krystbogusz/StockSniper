...
async def add_item(request: ItemRequest):
    ...
    if not is_safe:
        ...
        raise HTTPException(
            status_code=400, detail=f"URL rejected by security check: {reason}"
        )
...
async def update_item(request: ItemUpdateRequest):
    ...
    if not is_safe:
        api_logger.warning(
            f"URL rejected by security check for ID {request.id}: {reason}"
        )
        raise HTTPException(
            status_code=400, detail=f"URL rejected by security check: {reason}"
        )
...
async def delete_item(request: ItemDeleteRequest):
    api_logger.info(f"Received request to delete item ID: {request.id}")
    success, message = item_manager.delete_item(request.id, request.size)
    if not success:
        api_logger.warning(f"Failed to delete item {request.id}: {message}")
        raise HTTPException(status_code=404, detail=message)

    api_logger.info(f"Item deleted successfully: {request.id}")
    return ItemActionResponse(success=success, message=message)
